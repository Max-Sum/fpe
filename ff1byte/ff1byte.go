...
// package ff1byte implements the FF1 format-preserving encryption
// algorithm/scheme with 256 radix
package ff1byte
...
// based on bytes stream, max tweak length, key and tweak parameters.
func NewCipher(maxTLen int, key []byte, tweak []byte) (Cipher, error) {
	...
	// Calculate minLength
	minLen := uint32(math.Ceil(math.Log(feistelMin) / math.Log(float64(64))))
...
// Encrypt encrypts the []byte X over the current FF1 parameters
// and returns the ciphertext of the same length
...
func (c Cipher) EncryptWithTweak(X []byte, tweak []byte) ([]byte, error) {
	...
	ret = make([]byte, n)
	copy(ret[int(u)-len(A):int(u)], A[:])
	copy(ret[int(v)-len(B)+int(u):], B[:])

	return ret, nil
}

// Decrypt decrypts the []byte X over the current FF1 parameters
// and returns the plaintext of the same length
